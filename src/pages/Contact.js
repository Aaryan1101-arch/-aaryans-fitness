...
//   REACT_APP_EMAILJS_SERVICE_ID   — from emailjs.com → Email Services
//   REACT_APP_EMAILJS_TEMPLATE_ID  — from emailjs.com → Email Templates
//   REACT_APP_EMAILJS_PUBLIC_KEY   — from emailjs.com → Account → API Keys
const EJ_SERVICE  = process.env.REACT_APP_EMAILJS_SERVICE_ID;
const EJ_TEMPLATE = process.env.REACT_APP_EMAILJS_TEMPLATE_ID;
  ...
  return (
    <section id="contact" className="section bg-ink-900 overflow-hidden">
      ...
      <div className="flex flex-col-reverse form:flex-row items-stretch justify-center gap-10 lg:gap-16 max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {contact.phonePrimary && (
              <a href={`tel:${contact.phonePrimary.replace(/\s+/g, "")}`} className="glass-card flex items-center gap-3 p-4">
                <span className="w-10 h-10 rounded-full bg-brand/20 text-brand flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  <p className="text-xs text-white/40 uppercase tracking-wider">Call</p>
                  <p className="text-white text-sm">{contact.phonePrimary}</p>
                </div>
              </a>
            )}
            {contact.email && (
              <a href={`mailto:${contact.email}`} className="glass-card flex items-center gap-3 p-4">
                <span className="w-10 h-10 rounded-full bg-brand/20 text-brand flex items-center justify-center flex-shrink-0">
                  <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.8" stroke="currentColor" className="w-5 h-5">
        ...
        {/* Form — or a "contact us directly" fallback if EmailJS not configured */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.2 }}
        ...
        >
          {emailConfigured ? (
            <form ref={form} onSubmit={sendEmail}>
              ...
              <FormField label="Contact Number" id="user_number" name="user_number" required placeholder="+977 …" />
              <FormField label="Message"        id="message"     name="message"     required multiline placeholder="What can we help you with?" />
              <button
                type="submit"
                disabled={loading}
            ...
            /* Shown when EmailJS env vars aren't set yet */
            <div className="glass-card p-8 sm:w-80 lg:w-96 text-center">
              <div className="w-12 h-12 rounded-full bg-brand/20 text-brand flex items-center justify-center mx-auto mb-4">
                <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.8" stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                </svg>
              </div>
              <p className="text-white font-medium mb-2">Reach us directly</p>
              <p className="text-white/60 text-sm mb-6 leading-relaxed">
                Use the phone or email on the left, or visit us at the gym.
              </p>
              {contact.phonePrimary && (
                <a href={`tel:${contact.phonePrimary.replace(/\s+/g, "")}`} className="button w-full justify-center mb-3">
                  Call {contact.phonePrimary}
                </a>
              )}
              {contact.email && (
                <a href={`mailto:${contact.email}`} className="button-ghost w-full justify-center">
                  {contact.email}
                </a>
              )}
            </div>
          ...
          <AnimatePresence>
            {toast && (
              ...
              >
                {toast.kind === "success" ? (
                  <svg fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-5 h-5 flex-shrink-0">
